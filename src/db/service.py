"""ビジネスロジックを提供するサービスクラス。

このモジュールは、CSV インポートなどのビジネスロジックを
カプセル化するサービスクラスを提供します。
"""
...
class CountryService:
    """国データに関するビジネスロジックを提供するサービス。

    Attributes:
        repository: 国データのリポジトリ。
        session: データベースセッション。
    """

    def __init__(self, repository: CountryRepository, session: AsyncSession) -> None:
        """サービスを初期化する。
